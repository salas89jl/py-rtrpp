...
class RPLidarGetHealthData:
    """ Represents a response packet that is parsed from bytes received from the RPLIDAR device in GET_HEALTH mode. """
    status: int
    error_code: int
...
def parse_scan_data(packet: bytes) -> RPLidarScanData:
    """ Decodes the raw scan data received from the RPLIDAR device using SCAN request into a structured format. """
    if len(packet) != 5:
    ...
    if start_flag == inverse_start_flag:
        raise ValueError("Invalid start flag in scan packet. ")
