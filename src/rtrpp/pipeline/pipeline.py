...
def run_pipeline(file_path):
    ...
    downsampled = voxel_filter.downsample_point_cloud(xyz=roi_filtered[:, :3], voxel_size=0.2)

    # Remove outliers from downsampled points
    cleaned_points = outlier_filter.remove_outliers(downsampled, nb_neighbors=20, std_ratio=2.0)
    ...
    labels = dbscan_cluster.cluster_objects(object_points, eps=1.0, min_points=13)
